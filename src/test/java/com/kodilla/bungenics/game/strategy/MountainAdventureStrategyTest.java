...
class MountainAdventureStrategyTest {
    ...
    @InjectMocks
    private MountainAdventureStrategy strategy;
    ...
    @Test
    void shouldReturnMountainAdventureType() {
        assertEquals("MOUNTAIN", strategy.getAdventureType());
    }
    ...
    class ExecuteAdventure {
        ...
        void shouldBuildFullSuccessfulAdventure() {
            ...
            when(weatherFetcher.fetchCurrentWeatherForCity("Zakopane")).thenReturn(record);

            Player player = new Player();
            player.setLocation("Zakopane");
            ...
            try (MockedStatic<AdventureHelper> helper = mockStatic(AdventureHelper.class, CALLS_REAL_METHODS)) {
                ...
                verify(weatherFetcher).fetchCurrentWeatherForCity("Zakopane");
    ...
    class WeatherMappingAndPools {
        ...
        void shouldBuildCorrectPoolsForHeatWithAllTriggers() {
            ...
            try (MockedStatic<AdventureHelper> helper = mockStatic(AdventureHelper.class, CALLS_REAL_METHODS)) {
                ...
                assertTrue(neutralNames.contains("Sun-Baked Crags"));
                assertTrue(neutralNames.contains("Glacial Stream Refresh"));
                assertTrue(neutralNames.contains("Alpine Gale Gusts"));
                assertTrue(neutralNames.contains("Mountain Ridge Fog"));
                assertTrue(neutralNames.contains("Sunlit Peak Panorama"));

                // Skill pool: 3 core + 1 heat (Strength Test - Scorching Summit Climb) + 1 wind(>25) + 1 humidity(>80) = 6
                ...
                assertTrue(skillNames.contains("Strength Test - Scorching Summit Climb"));
                assertTrue(skillNames.contains("Agility Test - Peak Gale Balance"));
                assertTrue(skillNames.contains("Intelligence Test - Mist-Shrouded Ravine"));
        ...
        void shouldHandleSnowWeather() {
            ...
            try (MockedStatic<AdventureHelper> helper = mockStatic(AdventureHelper.class, CALLS_REAL_METHODS)) {
                ...
                assertTrue(names.contains("Blizzard Ridge"));
                assertTrue(names.contains("Icy Stalactite View"));
            }
        }
        ...
        void shouldHandleStormWeather() {
            ...
            try (MockedStatic<AdventureHelper> helper = mockStatic(AdventureHelper.class, CALLS_REAL_METHODS)) {
                ...
                List<String> names = new ArrayList<>();
                neutralPool.forEach(s -> names.add(s.get().getName()));
                assertTrue(names.contains("Mountain Lightning"));
                assertTrue(names.contains("Rock Cave Haven"));
                assertTrue(names.contains("Alpine Gale Gusts"));
                assertTrue(names.contains("Mountain Ridge Fog"));
            }
        }
    }
}